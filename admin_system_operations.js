...
import { db, currentUser, globalAiSystemPrompts, setGlobalAiSystemPrompts } from './state.js';
import { ADMIN_UID } from './config.js';
import { escapeHtml, showLoading, hideLoading } from './utils.js';
import { saveGlobalAiPrompts, fetchAdminTasks, addAdminTask, updateAdminTaskStatus, deleteAdminTask } from './firebase_firestore.js'; // Assuming these exist
...
export async function handleAddAdminTask() {
    ...
    try {
        const newTaskId = await addAdminTask(taskText); // From firebase_firestore.js
        if (newTaskId) { input.value = ''; loadAdminTasksUI(); }
    } catch (error) { console.error("Error in handleAddAdminTask UI:", error); /* Firestore func alerts */ }
    finally { hideLoading(); }
}

export async function handleToggleAdminTaskStatus(taskId, isCurrentlyDone) { // Renamed from ui_admin_dashboard
    if (!currentUser || currentUser.uid !== ADMIN_UID) {
    ...
    try {
        const success = await updateAdminTaskStatus(taskId, newStatus); // From firebase_firestore.js
        if (success) loadAdminTasksUI();
    } catch (error) { console.error("Error in handleToggleAdminTaskStatus UI:", error); /* Firestore func alerts */ }
    finally { hideLoading(); }
}
...
export async function handleDeleteAdminTask(taskId) {
    ...
    try {
        const success = await deleteAdminTask(taskId); // From firebase_firestore.js
        if (success) loadAdminTasksUI();
    } catch (error) { console.error("Error in handleDeleteAdminTask UI:", error); /* Firestore func alerts */ }
    finally { hideLoading(); }
}


// --- Global AI System Prompts Management ---
function formatAiFunctionKeyForDisplay(key) { // Moved local to this module
...
export async function handleSaveGlobalPromptsToFirestore() { // Renamed from ui_admin_dashboard
    if (!currentUser || currentUser.uid !== ADMIN_UID) {
    ...
    try {
        const success = await saveGlobalAiPrompts(newGlobalPrompts); // from firebase_firestore.js
        if (success) {
            setGlobalAiSystemPrompts(newGlobalPrompts);
            alert("Global AI System Prompts saved successfully!");
            renderGlobalAiPromptsAdminUI();
        } // Else, Firestore func alerts
    } catch (error) { console.error("Error in handleSaveGlobalPromptsToFirestore UI:", error); /* Firestore func alerts */ }
    finally { hideLoading(); }
}

// --- Chat Auto-Deletion Settings ---
export async function loadChatAutoDeleteSettingAdmin() { // Renamed from ui_admin_dashboard
...
export async function saveChatAutoDeleteSettingAdmin() { // Renamed from ui_admin_dashboard
    if (!currentUser || currentUser.uid !== ADMIN_UID) {
    ...
    containerElement.innerHTML = `
        ...
        <div class="space-y-8">
            ...
            <section class="p-4 border dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-700/30">
                ...
                <div id="admin-tasks-area-content" class="max-h-96 overflow-y-auto pr-2">
