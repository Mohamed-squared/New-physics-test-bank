...
export async function startLectureTranscription(youtubeUrl, courseId, chapterId) {
    ...
    // Removed AssemblyAI API Key prompt
    const megaEmail = prompt("Enter your MEGA Email:");
    if (!megaEmail) {
        feedbackArea.innerHTML = `<p class="text-yellow-600 dark:text-yellow-400">MEGA Email is required to archive transcripts.</p>`;
        return;
    }
    const megaPassword = prompt("Enter your MEGA Password:");
    if (!megaPassword) {
        feedbackArea.innerHTML = `<p class="text-yellow-600 dark:text-yellow-400">MEGA Password is required to archive transcripts.</p>`;
        return;
    }
    ...
    try {
        const response = await fetch('http://localhost:3001/transcribe-lecture', {
            ...
            body: JSON.stringify({ youtubeUrl, courseId, chapterId, assemblyAiApiKey: ASSEMBLYAI_API_KEY, megaEmail, megaPassword }),
        });
        hideLoading();
