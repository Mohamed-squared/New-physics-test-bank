...
import { currentUser, db, globalCourseDataMap, userCourseProgressMap } from './state.js';
...
export async function handlePaceSelection(event, courseId) {
    ...
    const initialProgressData = {
        courseId: courseId,
        enrollmentDate: firebase.firestore.FieldValue.serverTimestamp(), // Use server timestamp
    ...
    if (success) {
        ...
        initialProgressData.enrollmentDate = new Date(); // Use client time for local state
        initialProgressData.lastActivityDate = new Date();
        userCourseProgressMap.set(courseId, initialProgressData);
        setActiveCourseId(courseId); // Set this as the active course
