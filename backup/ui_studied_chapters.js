...
export function showManageStudiedChapters() {
    ...
    if (chapterNumbers.length === 0) {
        displayContent('<p class="text-red-500 p-4">No chapters with questions found for this subject.</p>');
        return;
    }
    ...
    const html = `
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-4">
            <h2 class="text-xl font-semibold mb-4 text-primary-600 dark:text-primary-400">Manage Studied Chapters (${currentSubject.name || 'Unnamed Subject'})</h2>
    ...
    `;
    displayContent(html);
    setActiveSidebarLink('showManageStudiedChapters');
}
...
export async function toggleStudiedChapter(chapNum, isChecked) {
    ...
    console.log(`toggleStudiedChapter called for chapNum: ${chapNum}, isChecked: ${isChecked}`);
     ...
     if (!currentUser || !currentSubject || !data) { // Added check for 'data'
         console.error("Cannot toggle studied chapter: State missing (currentUser, currentSubject, or data).");
         // Optionally revert checkbox state visually if possible
    ...
    if (changed) {
        ...
        try {
            await saveUserData(currentUser.uid); // saveUserData should save the entire 'data' object
