...
import { showCourseStudyMaterial, triggerSkipExamGeneration, calculateChapterCombinedProgress, getYouTubeVideoId, videoDurationMap } from './ui_course_study_material.js'; // Added videoDurationMap import
...
export async function displayCourseContentMenu(courseId = activeCourseId) {
    ...
    if (totalChapters === 0) {
        ...
        for (let i = 1; i <= totalChapters; i++) {
            ...
            const transcriptionPath = hasTranscriptionSource ? "exists" : null; // Simplified check
            const canAttemptSkip = pdfPath || transcriptionPath;

            // --- Skip Exam Button Logic ---
             let skipExamStatusHtml = '';
             if (isStudied) {
                  // If studied, show score if available, otherwise just "Studied"
                  skipExamStatusHtml = `<span class="text-xs text-green-600 dark:text-green-400 font-medium ml-auto">${lastSkipScore !== undefined ? `(Skip Exam Passed: ${lastSkipScore.toFixed(0)}%)` : ''}</span>`;
             } else if (lastSkipScore !== undefined && lastSkipScore < SKIP_EXAM_PASSING_PERCENT) {
                  // If failed last attempt, allow retry
                  ...
                  // If never attempted or previous pass doesn't matter (if not yet 100% progress), show Take button
