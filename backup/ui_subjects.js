...
export function showManageSubjects() {
     if (!data || typeof data.subjects !== 'object') {
         displayContent('<p class="text-danger p-4 text-center">Error: Subject data structure not loaded. Please try reloading.</p>');
         return;
     }
        ...
        ? subjectEntries.map(([id, subject]) => `
            <div class="flex flex-wrap justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-md shadow-sm gap-2 mb-2 border dark:border-gray-600 transition-colors hover:bg-gray-100 dark:hover:bg-gray-600">
                 <div class="flex-grow min-w-0">
                    <span class="font-semibold ${currentSubject && currentSubject.id === id ? 'text-primary-600 dark:text-primary-400' : 'text-gray-800 dark:text-gray-200'}" title="${subject.name || 'Unnamed'}">
                        ${subject.name || 'Unnamed Subject'}
                    </span>
                    <span class="block text-xs text-muted truncate" title="${subject.fileName || 'chapters.md'}">(${subject.fileName || 'chapters.md'})</span>
                 </div>
     ...
     let output = `
        <div class="animate-fade-in space-y-6">
            <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">Manage Subjects</h2>
    ...
    setActiveSidebarLink('showManageSubjects');
}
...
export function editSubject(id) {
    ...
    displayContent(`
        <div class="animate-fade-in">
            <h2 class="text-2xl font-semibold mb-6 text-gray-800 dark:text-gray-200">Edit Subject: <span class="text-primary-600 dark:text-primary-400">${subject.name || 'Unnamed'}</span></h2>
            <form id="edit-subject-form" onsubmit="window.updateSubject(event, '${id}')" class="space-y-4">
                 <div>
                     <label for="edit-subject-name">Subject Name</label>
                     <input id="edit-subject-name" type="text" value="${subject.name || ''}" required placeholder="Enter subject name">
                 </div>
                  <div>
                      <label for="edit-subject-filename">Markdown Filename</label>
                      <input id="edit-subject-filename" type="text"
                             value="${subject.fileName || ''}"
    ...
    setActiveSidebarLink('showManageSubjects'); // Keep subject link active
}


// --- Functions: selectSubject, updateSubject, addSubject, confirmDeleteSubject, deleteSubject ---
// (Keep the internal logic the same as previously provided)
// Ensure setActiveSidebarLink calls are present as needed.
// Example selectSubject:
...
// ... (addSubject, updateSubject, confirmDeleteSubject, deleteSubject remain the same as the previous correct version) ...
export async function addSubject(event) {
    event.preventDefault();
    if (!currentUser || !data) { /* ... validation ... */ return; }
    ...
    if (!fileName || !fileName.endsWith('.md') || /[\/\\]/.test(fileName)) { /* ... */ hideLoading(); alert("Invalid filename..."); filenameInput?.focus(); return; }
    if (fileName.toLowerCase() === 'chapters.md' && Object.keys(data.subjects || {}).length > 0 && !(Object.keys(data.subjects).length === 1 && data.subjects["1"]?.name === "Fundamentals of Physics")) { /* ... */ hideLoading(); alert("'chapters.md' reserved..."); filenameInput?.focus(); return; }
    if (!name || isNaN(max_questions) || max_questions < 1) { /* ... */ hideLoading(); alert("Invalid name or max questions..."); return; }
    ...
    try {
        ...
        const successMsgHtml = `<div class="toast-notification toast-success animate-fade-in"><p class="font-medium">Subject "${name}" added successfully!</p><p class="text-xs">Chapters will load on next refresh.</p></div>`;
...
export async function updateSubject(event, id) {
     event.preventDefault();
     if (!currentUser || !data || !data.subjects || !data.subjects[id]) { /* ... */ return; }
    ...
    // Validation...
    if (!newFilename || !newFilename.endsWith('.md') || /[\/\\]/.test(newFilename)) { /* ... */ alert("Invalid filename..."); return; }
    ...
    try {
        ...
        if (currentSubject && currentSubject.id === id) {
             setCurrentSubject(subjectToUpdate); // Update current subject state if it's the one being edited
             updateSubjectInfo();
        }
        ...
        if (needsDataReload) {
             ...
             // Show success toast
             const successMsgHtml = `<div class="toast-notification toast-success animate-fade-in"><p>Subject '${newName}' updated!</p></div>`;
    ...
    displayContent(`
        <div class="bg-danger-100 dark:bg-red-900/30 p-6 rounded-lg text-center max-w-lg mx-auto border border-danger-300 dark:border-red-700 shadow-lg animate-fade-in">
            ...
            <p class="text-gray-700 dark:text-gray-300 mb-5">
                Permanently delete subject "<strong>${subject.name || 'Unnamed Subject'}</strong>"?<br><br>
...
export async function deleteSubject(id) {
     if (!currentUser || !data || !data.subjects) { /* ... */ return; }
     if (Object.keys(data.subjects).length <= 1) { /* ... */ return; }
    ...
    try {
        ...
        if (currentSubject && currentSubject.id === id) {
            ...
            setCurrentSubject(otherSubjectId ? data.subjects[otherSubjectId] : null);
            updateSubjectInfo();
        }
        ...
        // Show feedback Toast
         const successMsgHtml = `<div class="toast-notification toast-warning animate-fade-in"><p>Subject "${deletedSubjectName}" deleted.</p></div>`;
         const msgContainer = document.createElement('div'); msgContainer.innerHTML = successMsgHtml; document.body.appendChild(msgContainer); setTimeout(() => { msgContainer.remove(); }, 5000);
