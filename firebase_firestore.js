...
import { initialSubjectData, ADMIN_UID, DEFAULT_PROFILE_PIC_URL, FOP_COURSE_ID, FOP_COURSE_DEFINITION, GRADING_WEIGHTS, PASSING_GRADE_PERCENT } from './config.js'; // Added grading config
import { updateSubjectInfo } from './ui_core.js';
import { showOnboardingUI } from './ui_onboarding.js';
import { determineTodaysObjective, calculateTotalMark, getLetterGrade } from './course_logic.js'; // Added grading logic
...
export async function saveUserCourseProgress(uid, courseId, progressData) {
    ...
    try {
        ...
        // Use server timestamp for lastActivityDate when saving, ONLY if not already set in the transaction
        if (dataToSave.lastActivityDate instanceof Date){
             dataToSave.lastActivityDate = firebase.firestore.Timestamp.fromDate(dataToSave.lastActivityDate);
        } else if (!dataToSave.lastActivityDate) {
             // If it wasn't set at all (e.g., during initial enroll), use server timestamp now
        ...
        dataToSave.skipExamScores = dataToSave.skipExamScores || {};
 ...
 * @param {string} targetUserId - The UID of the user whose progress is being updated.
 * @param {string} courseId - The ID of the course.
 * @param {number | null} finalMark - The final numerical mark (0-100+). Set null if just changing status without final grade.
 * @param {'completed' | 'failed' | 'enrolled'} newStatus - The new status for the course.
...
export async function updateCourseStatusForUser(targetUserId, courseId, finalMark, newStatus) {
    ...
    try {
        ...
        // **FIXED:** Check against firebaseAuth.currentUser.uid directly
 ...
 * NOTE: This function uses arrayUnion which doesn't require a transaction,
 * and uses Timestamp.now() which is allowed.
 */
export async function handleAddBadgeForUser(userId, courseId, courseName, grade, completionDate) {
     ...
     try {
         const newBadge = {
             ...
             // Use Timestamp.now() or fromDate(clientDate) - serverTimestamp is not allowed in arrayUnion
             completionDate: completionDate ? firebase.firestore.Timestamp.fromDate(completionDate) : firebase.firestore.Timestamp.now()
         };
         // Check if badge already exists to prevent duplicates (optional but good practice)
         ...
         // Refresh the badge list if the admin panel is viewing this user
         const searchInput = document.getElementById('admin-user-search-badges');
         // Check if searchInput exists and its value matches the target userId
         if (searchInput && searchInput.value === userId) {
             window.loadUserBadgesForAdmin(); // Refresh the displayed list
         }
     } catch (error) {
         hideLoading();
 ...
 * Uses arrayRemove, which is allowed outside transactions.
 */
export async function handleRemoveBadgeForUser(userId, courseId) {
     ...
     try {
         ...
         // Use arrayRemove to remove the specific badge object
         await userRef.update({
             completedCourseBadges: firebase.firestore.FieldValue.arrayRemove(badgeToRemove)
         });
