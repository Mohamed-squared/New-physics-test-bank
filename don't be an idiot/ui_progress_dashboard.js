...
// Import Test Gen Dashboard function
...
export function showProgressDashboard() {
    ...
    if (!chapters || Object.keys(chapters).length === 0) {
         dashboardContentEl.innerHTML = `<p class="text-red-500 p-4 text-center">No chapter data available for subject "${escapeHtml(currentSubject.name) || 'Unnamed'}" to display the dashboard.</p>`;
         setActiveSidebarLink('showProgressDashboard', 'sidebar-standard-nav'); // Still set active link
         return;
    }
    ...
    if (chaptersWithQuestions.length === 0) {
         dashboardContentEl.innerHTML = `<p class="text-yellow-500 p-4 text-center">No chapters with questions defined for subject "${escapeHtml(currentSubject.name) || 'Unnamed'}". Cannot display progress.</p>`;
         setActiveSidebarLink('showProgressDashboard', 'sidebar-standard-nav'); // Still set active link
         return;
    }
    ...
    dashboardContentEl.innerHTML = `
     ...
     // Update the header icon and title
     const headerTitle = document.querySelector('#dashboard h2');
     if (headerTitle) {
          headerTitle.innerHTML = `
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
...
export function closeDashboard() {
    ...
    // MODIFIED: Navigate to Test Gen Dashboard instead of Home or clearing
    showTestGenerationDashboard();
    // setActiveSidebarLink is handled by showTestGenerationDashboard
}
