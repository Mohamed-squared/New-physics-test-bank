...
// MODIFIED: Import problem selection and combination logic
import { selectProblemsForExam, combineProblemsWithQuestions } from './test_logic.js';
// MODIFIED: Import notes panel function
...
export async function initPdfViewer(pdfPath) {
    ...
    const pdfExplainWholeButton = document.getElementById('pdf-explain-whole-button'); // Get new button

    if (!pdfViewerContainer || !pdfControls || !pdfExplainButton || !pdfExplainWholeButton) {
    ...
    pdfExplainWholeButton.disabled = true; // Disable initially
    ...
    try {
        ...
        loadingTask.promise.then(async (loadedPdfDoc) => {
            ...
            pdfExplainWholeButton.disabled = false; // Enable whole PDF button
            ...
            console.error(`Error during pdfjsLib.getDocument('${encodedPdfPath}'):`, error.name, error.message);
             ...
             pdfControls.classList.add('hidden');
             pdfExplainButton.disabled = true;
             pdfExplainWholeButton.disabled = true; // Disable too
             cleanupPdfViewer();
        });
...
window.handlePdfSnapshotForAI = handlePdfSnapshotForAI; // Assign to window

// NEW: Ask AI about the *entire* PDF document
async function askAboutWholePdf() {
    if (!pdfDoc || !currentCourseIdInternal || !currentChapterNumber) {
        alert("PDF not loaded or context missing.");
        return;
    }
    const userQuestion = prompt(`Ask a question about the entire PDF document for Chapter ${currentChapterNumber}:`);
    if (!userQuestion || userQuestion.trim() === "") return;

    showLoading(`Extracting text from entire PDF (Chapter ${currentChapterNumber})...`);
    const explanationArea = document.getElementById('ai-explanation-area');
    const explanationContent = document.getElementById('ai-explanation-content');
    if (!explanationArea || !explanationContent) { hideLoading(); return; }
    explanationContent.innerHTML = `<div class="flex items-center justify-center space-x-2"><div class="loader animate-spin rounded-full h-4 w-4 border-t-2 border-b-2 border-purple-500"></div><p class="text-sm">Extracting PDF text...</p></div>`;
    explanationArea.classList.remove('hidden');

    try {
        // Use existing extraction logic, but ensure we get the path correctly
        ...
        if (!pdfPath) throw new Error("Could not determine PDF path.");

        // Use the cached text if already extracted for summary/formula sheet, otherwise extract now
        let fullPdfText = currentPdfTextContent; // Use module-level cache first
        if (!fullPdfText) {
            fullPdfText = await getAllPdfTextForAI(pdfPath);
            currentPdfTextContent = fullPdfText; // Cache it
        }

        if (!fullPdfText) throw new Error("Failed to extract text from PDF.");

        showLoading("Generating AI explanation about the document...");
        explanationContent.innerHTML = `<div class="flex items-center justify-center space-x-2"><div class="loader animate-spin rounded-full h-4 w-4 border-t-2 border-b-2 border-purple-500"></div><p class="text-sm">Generating explanation...</p></div>`;

        // Call AI to explain based on the full text
        const context = `Entire PDF document for Chapter ${currentChapterNumber}.`;
        const explanationHtml = await explainStudyMaterialSnippet(userQuestion, context + `\n\nFull PDF Text:\n---\n${fullPdfText}\n---`); // Pass full text
        ...
        hideLoading();
        console.error("Error asking about whole PDF:", error);
        explanationContent.innerHTML = `<p class="text-danger text-sm">Error generating explanation: ${error.message}</p>`;
    }
}
window.askAboutWholePdf = askAboutWholePdf; // Assign to window scope
...
export async function showCourseStudyMaterial(courseId, chapterNum, videoIdx = 0) {
    ...
    setActiveSidebarLink('sidebar-study-material-link', 'sidebar-course-nav'); // Use correct link ID
    ...
    if (pdfPath) {
         // MODIFIED: Added "Ask AI About Doc" button
         studyMaterialHtml += `
            <div id="pdf-viewer-wrapper" class="mb-4">
                <div id="pdf-viewer-container" class="h-[70vh] max-h-[800px] mb-2 bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 overflow-auto shadow-inner rounded"></div>
            ...
            <div class="flex flex-wrap gap-2">
                 <button id="pdf-explain-button" onclick="window.handlePdfSnapshotForAI()" class="btn-secondary-small text-xs" disabled>Ask AI About Page</button>
                 <button id="pdf-explain-whole-button" onclick="window.askAboutWholePdf()" class="btn-secondary-small text-xs" disabled>Ask AI About Document</button>
                 <a href="${escapeHtml(encodeURI(pdfPath))}" target="_blank" class="link text-xs ml-auto self-center">(Open PDF in new tab)</a>
            </div>`;
     } else { studyMaterialHtml += `<p class="text-sm text-muted">No PDF available for this chapter.</p>`; }
    studyMaterialHtml += `</div>`;

    // Actions & Resources Section
    // MODIFIED: Added "View/Manage Notes" button
    studyMaterialHtml += `<div class="content-card"> <h3 class="text-lg font-semibold mb-3 text-gray-700 dark:text-gray-300">Actions & Resources</h3> <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">`;
    const canUseAI = (currentTranscriptionData && currentTranscriptionData.length > 0) || pdfPath;

    studyMaterialHtml += `<button onclick="window.showNotesDocumentsPanel('${courseId}', ${chapterNum})" class="btn-secondary flex flex-col items-center justify-center p-3 min-h-[5rem]">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-6 h-6 mb-1"><path d="M5.433 13.917l1.262-3.155A4 4 0 0 1 7.58 9.42l6.92-6.918a2.121 2.121 0 0 1 3 3l-6.92 6.918c-.383.383-.84.685-1.343.886l-3.154 1.262a.5.5 0 0 1-.65-.65Z" /><path d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0 0 10 3H4.75A2.75 2.75 0 0 0 2 5.75v9.5A2.75 2.75 0 0 0 4.75 18h9.5A2.75 2.75 0 0 0 17 15.25V10a.75.75 0 0 0-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5Z" /></svg>
                            <span class="text-sm font-medium">Notes</span>
                          </button>`;
    studyMaterialHtml += `<button id="view-formula-btn" onclick="window.displayFormulaSheet('${courseId}', ${chapterNum})" class="btn-secondary flex flex-col items-center justify-center p-3 min-h-[5rem]" ${!canUseAI ? 'disabled' : ''} title="${canUseAI ? 'Generate formula sheet using AI' : 'Requires Transcription or PDF'}">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-6 h-6 mb-1"><path fill-rule="evenodd" d="M15.962 2.038a.75.75 0 0 1 .707.056l2.25 1.5a.75.75 0 0 1 0 1.212l-2.25 1.5a.75.75 0 0 1-.707.056l-2.25-1.5a.75.75 0 0 1 0-1.212l2.25-1.5ZM13 5.39l-2.25-1.5a.75.75 0 0 0-.707-.056l-2.25 1.5a.75.75 0 0 0 0 1.212l2.25 1.5a.75.75 0 0 0 .707-.056l2.25-1.5a.75.75 0 0 0 0-1.212ZM7.038 6.057a.75.75 0 0 1-.707-.056l-2.25-1.5a.75.75 0 0 1 0-1.212l2.25-1.5a.75.75 0 0 1 .707.056l2.25 1.5a.75.75 0 0 1 0 1.212l-2.25 1.5ZM15.962 9.038a.75.75 0 0 1 .707.056l2.25 1.5a.75.75 0 0 1 0 1.212l-2.25 1.5a.75.75 0 0 1-.707.056l-2.25-1.5a.75.75 0 0 1 0-1.212l2.25-1.5ZM13 12.39l-2.25-1.5a.75.75 0 0 0-.707-.056l-2.25 1.5a.75.75 0 0 0 0 1.212l2.25 1.5a.75.75 0 0 0 .707-.056l2.25-1.5a.75.75 0 0 0 0-1.212ZM7.038 13.057a.75.75 0 0 1-.707-.056l-2.25-1.5a.75.75 0 0 1 0-1.212l2.25-1.5a.75.75 0 0 1 .707.056l2.25 1.5a.75.75 0 0 1 0 1.212l-2.25 1.5Z" clip-rule="evenodd" /></svg>
                            <span class="text-sm font-medium">Formula Sheet</span>
                            <span class="text-xs text-muted">(AI)</span>
                          </button>`;
    studyMaterialHtml += `<button id="view-summary-btn" onclick="window.displayChapterSummary('${courseId}', ${chapterNum})" class="btn-secondary flex flex-col items-center justify-center p-3 min-h-[5rem]" ${!canUseAI ? 'disabled' : ''} title="${canUseAI ? 'Generate summary using AI' : 'Requires Transcription or PDF'}">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-6 h-6 mb-1"><path d="M3 4.5A1.5 1.5 0 0 1 4.5 3h11A1.5 1.5 0 0 1 17 4.5v11a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 3 15.5v-11Zm7 2.75a.75.75 0 0 0-1.5 0v4.5a.75.75 0 0 0 1.5 0v-4.5Z" /></svg>
                            <span class="text-sm font-medium">Chapter Summary</span>
                            <span class="text-xs text-muted">(AI)</span>
                          </button>`;
    studyMaterialHtml += `<button onclick="window.triggerSkipExamGeneration('${courseId}', ${chapterNum})" class="btn-warning flex flex-col items-center justify-center p-3 min-h-[5rem]" ${!canUseAI ? 'disabled' : ''} title="${canUseAI ? `Take exam (${SKIP_EXAM_PASSING_PERCENT}%) to achieve 100% progress` : 'Requires Transcription or PDF'}">
...
// ... (rest of the functions from the previous turn, like triggerSkipExamGeneration, handleExplainSelection, askQuestionAboutTranscription, displayFormulaSheet, downloadFormulaSheetPdf, displayChapterSummary, downloadChapterSummaryPdf, navigateChapterMaterial, switchVideo, fetchVideoDurationsIfNeeded) ...
// Need to copy them here for completeness of the file.

// (Copy functions from previous answer: triggerSkipExamGeneration, handleExplainSelectionInternal, handleExplainSelection, askQuestionAboutTranscription, displayFormulaSheet, downloadFormulaSheetPdf, displayChapterSummary, downloadChapterSummaryPdf, navigateChapterMaterial, switchVideo, fetchVideoDurationsIfNeeded)
...
export async function triggerSkipExamGeneration(courseId, chapterNum) {
    ...
    try {
        // Fetch content and generate MCQs using AI
        const examText = await generateSkipExam(courseId, chapterNum); // Pass courseId for context
        if (!examText) throw new Error("Failed to generate exam MCQs.");
        ...
        if (!parsedMCQs || parsedMCQs.questions.length === 0) {
             ...
             // Fallback or throw error? For now, let's proceed assuming problems might still exist
        }

        // Get predefined problems for the chapter
        const courseDef = globalCourseDataMap.get(courseId);
        const subjectId = courseDef?.relatedSubjectId;
        if (!subjectId) throw new Error("Related subject ID not found for course.");
        // We need access to the problem cache or the parsing function here
        // Assuming parseChapterProblems is available or called earlier
        // await parseChapterProblems(); // Ensure cache is warm if needed
...
window.triggerSkipExamGeneration = triggerSkipExamGeneration; // Assign to window
...
window.handleExplainSelection = handleExplainSelection; // Assign to window
...
window.askQuestionAboutTranscription = askQuestionAboutTranscription; // Assign to window
...
window.navigateChapterMaterial = navigateChapterMaterial; // Assign to window
