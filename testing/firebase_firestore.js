 ...
 */
export async function saveUserData(uid, appDataToSave = data) { // Accept data optionally
    if (!db) { console.error("Firestore DB not initialized"); return; }
    if (!uid || !appDataToSave) { // Check appDataToSave instead of global 'data'
    ...
    try {
        ...
        const cleanData = JSON.parse(JSON.stringify(appDataToSave)); // Use the provided data
...
export async function handleAddBadgeForUser(userId, courseId, courseName, grade, completionDate) {
     ...
     try {
         ...
         // Corrected ID for badge search input
         const searchInput = document.getElementById('admin-user-search-badges-input');
         if (searchInput && (searchInput.value === userId || searchInput.value.toLowerCase() === userDoc.data()?.email?.toLowerCase())) { window.findAndLoadUserBadges(); } // Changed function call
...
// --- END OF MODIFIED FILE firebase_firestore.js ---