...
import { currentOnlineTestState, setCurrentOnlineTestState, currentSubject, currentUser, data, setData, activeCourseId, userCourseProgressMap } from './state.js'; // Added course state
...
export function launchOnlineTestUI() {
    ...
    <div class="fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 p-3 shadow z-40 border-b dark:border-gray-700">
        <div class="container mx-auto flex justify-between items-center">
             <!-- MODIFIED: Display more context if available -->
...
export async function submitOnlineTest() {
    ...
    // MODIFIED: Check for course context existence if needed later
    if (!currentOnlineTestState || currentOnlineTestState.status === 'submitting' || !currentUser) {
        ...
        console.error("submitOnlineTest Error: State or user missing, or already submitting.");
        hideLoading(); // Ensure loading indicator is hidden
        return;
    }

    // Determine if this is a standard test or a course activity/skip exam
    const isCourseActivity = currentOnlineTestState.courseContext?.isCourseActivity ?? false;
    const isSkipExam = currentOnlineTestState.courseContext?.isSkipExam ?? false;
    ...
    const activityId = currentOnlineTestState.courseContext?.activityId; // e.g., day1, week1, mid1, final1
    const chapterNumForSkipExam = currentOnlineTestState.courseContext?.chapterNum; // Specific for skip exam

    // MODIFIED: Check for subject/data only if it's a standard test gen exam
    if (!isCourseActivity && (!currentSubject || !data)) {
         alert("Error submitting standard test: Subject data missing.");
         console.error("submitOnlineTest Error: Subject or appData missing for standard test.");
         hideLoading();
         return;
    }
    ...
    try {
        ...
        const results = {
            ...
            // MODIFIED: Get subjectId from course context or currentSubject
            subjectId: isCourseActivity ? globalCourseDataMap.get(courseId)?.relatedSubjectId : currentSubject?.id,
            ...
            // MODIFIED: Include course context if available
            courseContext: currentOnlineTestState.courseContext || null,
             // Standard test gen aggregation
             resultsByChapter: {}
        };

        let totalCorrect = 0;
        let chaptersDataModified = false; // Flag to check if standard subject chapter data needs saving
        ...
        currentOnlineTestState.questions.forEach(q => {
            ...
            // Update chapter-specific aggregates for standard test gen report
            ...
            if (chapResult) {
             ...
             // --- Remove question from available pool in the main 'data' object (ONLY for standard tests) ---
             if (!isCourseActivity && currentSubject) {
                 ...
                 if (globalChap && globalChap.available_questions && Array.isArray(globalChap.available_questions)) {
                      ...
                      if (qIndexInAvailable > -1) {
                          ...
                          console.warn(`Std Test: Question ${q.id} (num ${q.number}) not found in available_questions for Chapter ${q.chapter}.`);
                      }
                  } else { console.error(`Std Test: Chapter ${q.chapter} or its available_questions list missing.`); }
             }
        });

        // --- Update chapter mastery and mistake history (ONLY for standard tests) ---
        if (!isCourseActivity && currentSubject) {
            for (const chapNum in results.resultsByChapter) {
             ...
             // Sort available questions if modified
        ...
        const scorePercentage = results.totalQuestions > 0 ? (results.score / results.totalQuestions) * 100 : 0;
        ...
        if (isCourseActivity && courseId) {
            ...
            if (progress) {
                 // Update relevant score field based on activityType
                 switch (activityType) {
                      case 'skip_exam': // Specific handling for skip exam
                           ...
                           progress.lastSkipExamScore[chapterNumForSkipExam] = scorePercentage;
                           // Also store in the general skipExamScores map if used for grading
                           progress.skipExamScores = progress.skipExamScores || {};
                           progress.skipExamScores[chapterNumForSkipExam] = scorePercentage;
                           break;
                     case 'assignment':
                         progress.assignmentScores = progress.assignmentScores || {};
                         progress.assignmentScores[activityId] = scorePercentage;
                         // Mark assignment as completed in daily progress log?
                         // const dateStr = activityId.replace('day', ''); // Needs better parsing if format changes
                         // if(progress.dailyProgress[dateStr]) progress.dailyProgress[dateStr].assignmentCompleted = true;
                         break;
                     case 'weekly_exam':
                         progress.weeklyExamScores = progress.weeklyExamScores || {};
                         progress.weeklyExamScores[activityId] = scorePercentage;
                         break;
                     case 'midcourse':
                         progress.midcourseExamScores = progress.midcourseExamScores || {};
                         progress.midcourseExamScores[activityId] = scorePercentage;
                         break;
                     case 'final':
                         progress.finalExamScores = progress.finalExamScores || [];
                         const finalIndex = parseInt(activityId.replace('final', '')) - 1;
                         if (finalIndex >= 0 && finalIndex < 3) {
                             // Ensure array is long enough
                             while (progress.finalExamScores.length <= finalIndex) {
                                 progress.finalExamScores.push(null);
                             }
                             progress.finalExamScores[finalIndex] = scorePercentage;
                         }
                         break;
                     default:
                         console.warn("Unknown course activity type for score saving:", activityType);
                 }
                 updateUserCourseProgress(courseId, progress); // Update local state
                 ...
                 // If it was a skip exam and passed, mark the chapter studied
                 if (isSkipExam && scorePercentage >= SKIP_EXAM_PASSING_PERCENT) {
                      await markChapterStudiedInCourse(currentUser.uid, courseId, chapterNumForSkipExam, "skip_exam_pass");
                 } else if (isSkipExam) {
                      console.log(`Skip exam for Ch ${chapterNumForSkipExam} not passed (Score: ${scorePercentage}%, Needed: ${SKIP_EXAM_PASSING_PERCENT}%).`);
                      alert(`You scored ${scorePercentage.toFixed(1)}% on the skip exam for Chapter ${chapterNumForSkipExam}. You need ${SKIP_EXAM_PASSING_PERCENT}% to mark it as studied.`);
                 }

            } else { console.error(`Could not find progress data for course ${courseId} to save score.`); }
        } else if (currentSubject) {
             // Save standard test gen history
             currentSubject.exam_history = currentSubject.exam_history || [];
             currentSubject.exam_history.push(results);
             await saveUserData(currentUser.uid); // Saves the whole 'data' object
             console.log("Standard test results saved to exam_history and data updated.");
        }
        ...
        setCurrentOnlineTestState(null); // Clear the test state
         ...
         if (isCourseActivity) {
             window.showCurrentAssignmentsExams?.(courseId); // Go back to course assignments
         } else {
             showTestGenerationDashboard(); // Go back to standard test gen
         }
    }
}
...
export function displayOnlineTestResults(results) {
    ...
    // MODIFIED: Find in course history or standard history
    let examIndexInHistory = -1;
    let targetHistory = null;
    if(isCourse) {
         // Course activities aren't stored in exam_history in this structure
         // We might need a different way to review course activity details later
         console.log("Reviewing course activity results - detailed view might need separate implementation.");
    } else if (currentSubject?.exam_history) {
         targetHistory = currentSubject.exam_history;
         examIndexInHistory = targetHistory.findIndex(exam => exam.examId === results.examId && exam.timestamp === results.timestamp);
    }

     let reviewHtml = '';
     if (!isCourse && examIndexInHistory > -1) {
          // Provide a button to view the detailed results for standard tests
          reviewHtml = `
          ...
          `;
           ...
           reviewHtml = `<p class="text-sm text-gray-600 dark:text-gray-400 italic p-4 text-center bg-gray-100 dark:bg-gray-700 rounded">Detailed review for course activities coming soon.</p>`;
      } else {
    ...
    const html = `
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-4 animate-fade-in">
            <h2 class="text-2xl font-semibold mb-4 text-center text-primary-600 dark:text-primary-400">Test Results: ${isSkipExam ? `Chapter ${results.courseContext.chapterNum} Skip Exam` : results.examId}</h2>
             <p class="text-center text-sm text-gray-500 dark:text-gray-400 mb-4">${date}</p>
            <div class="text-center mb-6">
                 <p class="text-4xl font-bold ${scorePercentage >= (isSkipExam ? SKIP_EXAM_PASSING_PERCENT : 70) ? 'text-green-500' : scorePercentage >= 50 ? 'text-yellow-500' : 'text-red-500'}">
                    ${results.score} / ${results.totalQuestions} (${scorePercentage}%)
                 </p>
                 ${isSkipExam ? `<p class="text-sm font-medium ${scorePercentage >= SKIP_EXAM_PASSING_PERCENT ? 'text-green-600' : 'text-red-600'}">${scorePercentage >= SKIP_EXAM_PASSING_PERCENT ? 'Passed - Chapter Marked as Studied!' : `Failed (Needed ${SKIP_EXAM_PASSING_PERCENT}%)`}</p>` : ''}
                 ${results.durationMinutes !== null ? `<p class="text-gray-600 dark:text-gray-400 mt-1">Duration: ${results.durationMinutes} minutes</p>`: ''}
            </div>
            ...
            ${!isCourse ? `<button onclick="window.showTestGenerationDashboard()" class="w-full btn-primary mt-2">
    ...
    `;
    // MODIFIED: Display in main content area, not test area
    displayContent(html);
}
