...
async function fetchCourses(searchTerm = '', searchTag = '') {
    ...
    try {
        ...
        // --- START: FoP Injection Logic ---
        const fopInList = courses.some(c => c.id === FOP_COURSE_ID);
        if (!fopInList) {
             // Create a local copy of FoP definition, treat as approved
            const fopDef = {
        ...
        courses.forEach(course => {
            ...
            // FoP was potentially added with status 'approved' locally, so it passes this check
        ...
        // This filter WILL NOT remove FoP because we manually set its status to 'approved' if injected.
        if (currentUser?.uid !== ADMIN_UID) {
...
async function fetchCourseDetails(courseId) {
    ...
    if (globalCourseDataMap.has(courseId)) {
        ...
        // If non-admin trying to view a non-approved course (that isn't FoP), deny access
    ...
    if (courseId === FOP_COURSE_ID) {
        ...
        fopDef.id = FOP_COURSE_ID; // Ensure ID is set
...
function renderCourseList(courses, container) {
    ...
    container.innerHTML = courses.map(course => {
        ...
        // Skip rejected courses entirely unless admin (FoP won't be rejected due to fetch logic)
        ...
        const showFopAdminActions = isAdmin && course.id === FOP_COURSE_ID && (course.status === 'reported' || course.status === 'rejected'); // FoP special admin actions (Clear Report, Delete, Edit) - cannot be approved/rejected
        ...
        <div class="course-card border rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow duration-200 ${statusClass}">
            <div class="flex flex-col sm:flex-row justify-between items-start gap-3">
             ...
             ${showFopAdminActions ? `
                <div class="mt-3 pt-3 border-t dark:border-gray-600 text-right space-x-2">
                     ${course.status === 'reported' ? `<button onclick="window.handleCourseApproval('${course.id}', true)" class="btn-success-small">Clear Report</button><button type="button" onclick="alert('Report Reason: ${escapeHtml(course.reportReason || 'None provided')}')" class="btn-warning-small">View Report</button><button onclick="window.handleCourseApproval('${course.id}', false, true)" class="btn-danger-small">Delete Course</button>` : ''}
                     ${course.status === 'rejected' ? `<button onclick="window.handleCourseApproval('${course.id}', false, true)" class="btn-danger-small">Delete Course</button>` : ''}
                    <button onclick="window.showEditCourseForm('${course.id}')" class="btn-secondary-small">Edit</button>
                </div>
             ` : ''}
             ${isAdmin && course.id === FOP_COURSE_ID && !showFopAdminActions ? `
                <div class="mt-3 pt-3 border-t dark:border-gray-600 text-right space-x-2">
                     <button onclick="window.showEditCourseForm('${course.id}')" class="btn-secondary-small">Edit FoP Config</button>
...
export async function showEditCourseForm(courseId) {
    ...
    // Fetch directly to ensure we have the latest, even if it was 'rejected' or only in config
    let course = await fetchCourseDetails(courseId); // Use fetchCourseDetails as it handles fallback
...
export async function handleUpdateCourse(event, courseId) {
    ...
    if (updates.totalChapters !== currentTotalChapters || !currentChaptersArrayExists || currentChapterNames.length !== currentTotalChapters) {
         ...
         // No need to explicitly add 'updates.chapters = currentChapterNames' because Firestore update only changes specified fields.
    ...
    // If editing FoP, we update Firestore if it exists, otherwise it's just modifying the config representation in memory
    const isFoP = courseId === FOP_COURSE_ID;
    let messageAction = isFoP ? "FoP configuration representation" : `course "${updates.name}"`;


    // --- Add Logging ---
    console.log(`Attempting to update ${messageAction} (${courseId}) with data:`, JSON.stringify(updates, null, 2));
    // --- End Logging ---
    ...
    hideLoading();
    if (success) {
        alert(`${isFoP ? 'FoP configuration' : `Course "${updates.name}"`} updated successfully! ${isFoP ? '(Changes apply if FoP is used from Firestore or future config updates)' : ''}`);
...
export async function showCourseDetails(courseId) {
     ...
     const course = await fetchCourseDetails(courseId); // Uses the updated fetch which handles FoP fallback
     hideLoading();
     ...
     // Determine display status (FoP always shows as approved for non-admins)
     const displayStatus = (course.id === FOP_COURSE_ID && !isAdmin) ? 'approved' : course.status;
     ...
     const showAdminActions = isAdmin && course.id !== FOP_COURSE_ID && (course.status === 'pending' || course.status === 'reported' || course.status === 'rejected');
     const showFopAdminActions = isAdmin && course.id === FOP_COURSE_ID && (course.status === 'reported' || course.status === 'rejected'); // Actions possible if FoP exists in Firestore and is reported/rejected
...
export async function handleCourseApproval(courseId, approve, deleteCourse = false) {
    ...
    // Prevent approve/reject actions on FoP
    if (courseId === FOP_COURSE_ID && !deleteCourse && !(approve && (globalCourseDataMap.get(courseId)?.status === 'reported'))) { // Allow delete and clearing report for FoP
         alert("The Fundamentals of Physics course status cannot be directly approved or rejected here. Edit its config or clear reports if applicable.");
         return;
     }
    ...
    // Use fetchCourseDetails to handle FoP potentially not being in Firestore
    ...
    try {
        if (deleteCourse) {
            ...
            if (updatedDoc.exists) {
                 ...
                 globalCourseDataMap.delete(courseId);
                 if(courseId === FOP_COURSE_ID) { // If FoP was deleted from FS, ensure config view is available
                    const fopDef = {...FOP_COURSE_DEFINITION, id: FOP_COURSE_ID, status: 'approved'}; // Re-add from config, treated as approved
