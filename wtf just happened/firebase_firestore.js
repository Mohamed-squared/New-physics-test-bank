...
// CORRECTED: Import problem parser
import { updateChaptersFromMarkdown, updateChaptersWithProblems } from './markdown_parser.js';
 ...
 */
async function fetchMarkdownForSubject(subject, filenameOverride = null) {
     if (!subject && !filenameOverride) return null;
    // Use override first, then subject filename, then default logic
    const fileName = filenameOverride || subject?.fileName || (subject?.name === "Fundamentals of Physics" ? "chapters.md" : `${subject?.name || 'unknown'}.md`);
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                console.warn(`Markdown file not found: ${url}. Subject: ${subject?.name || 'N/A'}`);
        ...
        console.log(`Markdown fetched successfully for source ${fileName}.`);
        return mdContent;
    } catch (error) {
        console.error(`Error fetching Markdown for source ${fileName} (${url}):`, error);
...
// ... (saveUserCourseProgress, loadAllUserCourseProgress, unenrollFromCourse remain the same) ...
/**
...
export async function saveUserCourseProgress(uid, courseId, progressData) {
    ...
    try {
        ...
        if (dataToSave.enrollmentDate && !(dataToSave.enrollmentDate instanceof firebase.firestore.Timestamp)) { // Check if it's not already a timestamp
        ...
        if (dataToSave.completionDate && !(dataToSave.completionDate instanceof firebase.firestore.Timestamp)) {
        ...
        dataToSave.myNotes = dataToSave.myNotes || {}; // Ensure My Notes exists
    ...
    try {
        ...
        if (!snapshot.empty) {
            snapshot.forEach(doc => {
                ...
                progressData.myNotes = progressData.myNotes || {}; // Ensure My Notes exists
...
// --- NEW: Save Course Exam Result ---
/**
 * Saves the detailed results of a completed course activity (assignment, exam)
 * to a separate subcollection for review purposes.
 * @param {string} uid - The user's unique ID.
 * @param {object} examResultData - The detailed result object from submitOnlineTest.
 * @returns {Promise<boolean>} - True if successful, false otherwise.
 */
export async function saveCourseExamResult(uid, examResultData) {
    if (!db) { console.error("Firestore DB not initialized"); return false; }
    if (!uid || !examResultData || !examResultData.examId) {
        console.warn("Attempted to save course exam result without UID or exam data.");
        return false;
    }
    // Sanitize data before saving (e.g., remove excessively large fields if necessary)
    // For now, we assume examResultData is structured correctly.
    const resultRef = db.collection('userExamResults').doc(uid).collection('results').doc(examResultData.examId);
    try {
         // Convert timestamp string back to Date object for saving as Timestamp
         const dataToSave = {
             ...examResultData,
             timestamp: firebase.firestore.Timestamp.fromDate(new Date(examResultData.timestamp))
         };
        await resultRef.set(dataToSave);
        console.log(`Detailed course exam result saved successfully for exam ${examResultData.examId}.`);
        return true;
    } catch (error) {
        console.error(`Error saving detailed course exam result ${examResultData.examId}:`, error);
        // Don't alert the user for this background save unless critical
        return false;
    }
}

// --- Global Course Definitions ---
// ... (loadGlobalCourseDefinitions remains the same) ...
 ...
 * Syncs subject data with Markdown files (MCQ and Problems).
 * @param {string} uid - The user's unique ID.
 */
export async function loadUserData(uid) {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            if (data && data.subjects) {
                 ...
                 for (const subjectId in data.subjects) {
                     ...
                     if (subjectMarkdown !== null) {
                          ...
                          console.warn(`Skipping primary MD sync for Subject ${subject.name} (ID: ${subjectId}) - MD file missing.`);
                          // If primary MD is missing, should we clear MCQs? `updateChaptersFromMarkdown` handles this.
                     }

                     // 2. Sync with Problems Markdown (Fetch only once per filename)
                     const problemsFilename = "ChaptersProblems.md"; // Assuming one shared file for now
                     let problemsMarkdown = problemMdCache.get(problemsFilename);
                     if (problemsMarkdown === undefined) { // Not fetched yet
                         problemsMarkdown = await fetchMarkdownForSubject(null, problemsFilename);
                         problemMdCache.set(problemsFilename, problemsMarkdown); // Cache result (even if null)
                     }

                     if (problemsMarkdown !== null) {
                         const problemsModified = updateChaptersWithProblems(subject, problemsMarkdown);
                         if (problemsModified) { appDataWasModifiedBySync = true; }
                     } else {
                          console.warn(`Skipping problems MD sync for Subject ${subject.name} (ID: ${subjectId}) - ${problemsFilename} file missing.`);
                          // Ensure problem counts are zeroed out if the file is missing
                          let problemsZeroed = false;
                          for (const chapNum in subject.chapters) {
                               if (subject.chapters[chapNum].total_problems > 0) {
                                    subject.chapters[chapNum].total_problems = 0;
                                    subject.chapters[chapNum].available_problems = [];
                                    problemsZeroed = true;
                               }
                          }
                          if (problemsZeroed) appDataWasModifiedBySync = true;
                     }
                 }
             ...
             if (data && data.subjects) {
                for (const subjectId in data.subjects) {
                    ...
                    for (const chapNum in subject.chapters) {
                       ...
                       if (!chap) continue;
                       chap.total_questions = chap.total_questions ?? 0; // MCQs
                       chap.total_problems = chap.total_problems ?? 0; // Problems
                       ...
                       // Validate available_questions (MCQs)
                       const expectedAvailableMCQ = Array.from({ length: chap.total_questions }, (_, j) => j + 1);
                       const currentAvailableMCQSet = new Set(chap.available_questions || []);
                       const validAvailableMCQ = expectedAvailableMCQ.filter(qNum => currentAvailableMCQSet.has(qNum));
                       if (JSON.stringify(validAvailableMCQ.sort((a,b) => a-b)) !== JSON.stringify((chap.available_questions || []).sort((a,b)=>a-b))) {
                            chap.available_questions = validAvailableMCQ; needsAppDataSaveAfterLoad = true;
                       } else if (chap.available_questions === undefined) {
                            chap.available_questions = validAvailableMCQ; needsAppDataSaveAfterLoad = true;
                       }

                       // Validate available_problems
                       const expectedAvailableProb = Array.from({ length: chap.total_problems }, (_, j) => j + 1);
                       const currentAvailableProbSet = new Set(chap.available_problems || []);
                       const validAvailableProb = expectedAvailableProb.filter(pNum => currentAvailableProbSet.has(pNum));
                       if (JSON.stringify(validAvailableProb.sort((a,b) => a-b)) !== JSON.stringify((chap.available_problems || []).sort((a,b)=>a-b))) {
                            chap.available_problems = validAvailableProb; needsAppDataSaveAfterLoad = true;
                       } else if (chap.available_problems === undefined) {
                            chap.available_problems = validAvailableProb; needsAppDataSaveAfterLoad = true;
                       }
                    }
                }
            ...
            // Select Default Subject (unchanged)
            if (data && data.subjects) { /* ... */ } else { setCurrentSubject(null); updateSubjectInfo(); }
 ...
 * Includes initial sync with problem markdown file.
 */
export async function initializeUserData(uid, email, username, displayName = null, photoURL = null, forceReset = false) {
    ...
    if (!docExists || forceReset) {
        ...
        // Sync default subjects with their respective MD files (MCQ and Problems)
        const problemMdCache = new Map();
        for (const subjectId in defaultAppData.subjects) {
            ...
            if (defaultSubject) {
                ...
                // Sync Problems
                const problemsFilename = "ChaptersProblems.md";
                let problemsMarkdown = problemMdCache.get(problemsFilename);
                if (problemsMarkdown === undefined) {
                    problemsMarkdown = await fetchMarkdownForSubject(null, problemsFilename);
                    problemMdCache.set(problemsFilename, problemsMarkdown);
                }
                if (problemsMarkdown) { updateChaptersWithProblems(defaultSubject, problemsMarkdown); }
            }
        }
        ...
        try {
            ...
            if (usernameLower) { /* ... Reserve username ... */ }
        } catch (error) { /* ... Error handling ... */ }
    } else {
        // Update missing fields (unchanged logic)
        /* ... */
    }
}

// --- Onboarding Check ---
// ... (checkOnboarding remains the same) ...
...
// --- Feedback Management ---
// ... (submitFeedback remains the same) ...
...
// --- Inbox/Messaging ---
// ... (sendAdminReply, markMessageAsRead remain the same) ...
...
// ... (updateCourseDefinition remains the same) ...
              ...
              for (const chapNum in updates.chapterResources) {
                   if (currentData.chapterResources[chapNum]?.lectureUrls && updates.chapterResources[chapNum]?.lectureUrls) {
                        ...
                        if (!mergedData.chapterResources[chapNum]) {
                         ...
                         // Merge URLs, ensuring objects and removing duplicates based on URL
                         const existingUrls = new Map((currentData.chapterResources[chapNum].lectureUrls || []).map(lec => [lec.url, lec]));
                         (updates.chapterResources[chapNum].lectureUrls || []).forEach(lec => {
                             if(lec && lec.url) existingUrls.set(lec.url, lec); // Add or overwrite based on URL
                         });
                         mergedData.chapterResources[chapNum].lectureUrls = Array.from(existingUrls.values());
                   } else if (updates.chapterResources[chapNum]?.lectureUrls) {
                        // If only updates have lectureUrls, just use them
                        if (!mergedData.chapterResources[chapNum]) mergedData.chapterResources[chapNum] = {};
                        mergedData.chapterResources[chapNum].lectureUrls = updates.chapterResources[chapNum].lectureUrls;
                   }
              }
         }
...
// ... (markChapterStudiedInCourse remains the same) ...
...
// ... (updateCourseStatusForUser remains the same) ...
...
// --- Badge Management ---
// ... (handleAddBadgeForUser, handleRemoveBadgeForUser remain the same) ...
...
// --- NEW: Global Generated Content Storage ---
/**
 * Saves generated content (like formula sheets, summaries) globally for a course chapter.
 * @param {string} courseId
 * @param {number} chapterNum
 * @param {'formula_sheet' | 'summary'} contentType
 * @param {string} contentText - The raw generated text content.
 * @returns {Promise<boolean>}
 */
export async function saveStoredChapterContent(courseId, chapterNum, contentType, contentText) {
    if (!db || !courseId || !chapterNum || !contentType || !contentText) {
        console.warn("saveStoredChapterContent: Missing arguments.");
        return false;
    }
    // Only admins can save/overwrite generated content? Or allow first user to generate?
    // Let's allow anyone for now, but admin could overwrite later.
    if (!currentUser) {
         console.warn("Cannot save generated content: User not logged in.");
         return false;
    }

    const docId = `${contentType}_ch${chapterNum}`;
    const contentRef = db.collection('courses').doc(courseId).collection('generatedContent').doc(docId);
    ...
    try {
        await contentRef.set({
            ...
            generatedBy: currentUser.uid // Track who generated it last
        });
        console.log(`Saved generated ${contentType} for course ${courseId}, chapter ${chapterNum}.`);
        return true;
    } catch (error) {
        console.error(`Error saving generated ${contentType} for ${courseId}-Ch${chapterNum}:`, error);
        return false;
    }
}

/**
 * Retrieves previously generated and stored content for a course chapter.
 * @param {string} courseId
 * @param {number} chapterNum
 * @param {'formula_sheet' | 'summary'} contentType
 * @returns {Promise<string|null>} - The stored content text or null if not found/error.
 */
export async function getStoredChapterContent(courseId, chapterNum, contentType) {
    if (!db || !courseId || !chapterNum || !contentType) {
        console.warn("getStoredChapterContent: Missing arguments.");
        return null;
    }
    const docId = `${contentType}_ch${chapterNum}`;
    const contentRef = db.collection('courses').doc(courseId).collection('generatedContent').doc(docId);
    ...
    try {
        ...
        if (doc.exists) {
            console.log(`Retrieved stored ${contentType} for course ${courseId}, chapter ${chapterNum}.`);
            return doc.data().content;
        } else {
            console.log(`No stored ${contentType} found for course ${courseId}, chapter ${chapterNum}.`);
            return null;
        }
    } catch (error) {
        console.error(`Error getting stored ${contentType} for ${courseId}-Ch${chapterNum}:`, error);
        return null;
    }
}
