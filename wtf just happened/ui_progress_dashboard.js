...
export function showProgressDashboard() {
    ...
    dashboardContentEl.innerHTML = `
     ...
     setActiveSidebarLink('showProgressDashboard');
}
...
export function renderCharts() {
    ...
    const chapters = currentSubject.chapters;
    const chapterNumbers = Object.keys(chapters).filter(num => chapters[num]?.total_questions > 0);
    ...
    try {
        ...
        const commonOptions = (/* title */) => ({ // Title removed from options, set in HTML h3
            responsive: true,
        ...
        if (masteryCtx) {
            ...
            const masteryOptions = commonOptions('Consecutive Mastery');
        ...
        // Create Difficulty Chart (with color coding)
         if (difficultyCtx) {
            const difficultyOptions = commonOptions('Difficulty Score');
