...
export function showCourseProgressDetails(courseId = activeCourseId) {
    ...
    // --- CORRECTED: Calculate/Retrieve Final Grade/Mark ---
    let displayMark = null;
    let displayGrade = null;
    const isCourseCompletedOrFailed = progress.status === 'completed' || progress.status === 'failed';

    if (isCourseCompletedOrFailed) {
        // For completed/failed courses, prioritize the stored values
        displayMark = progress.totalMark !== null && progress.totalMark !== undefined ? progress.totalMark : calculateTotalMark(progress);
        displayGrade = progress.grade || getLetterGrade(displayMark); // Use stored grade, fallback to calculation
        console.log(`Course is ${progress.status}. Using stored/final values: Mark=${displayMark}, Grade=${displayGrade}`);
    } else {
        // For enrolled courses, calculate current progress
        displayMark = calculateTotalMark(progress);
        displayGrade = getLetterGrade(displayMark);
        console.log(`Course is ${progress.status}. Calculating interim values: Mark=${displayMark}, Grade=${displayGrade}`);
    }
    // --- End Correction ---

    // Calculate other metrics
    const gradeColor = getLetterGradeColor(displayGrade); // Use displayGrade
    ...
    // Use status from progress data consistently
    const isCourseCompleted = progress.status === 'completed'; // Keep this check based on actual status
    const completionDate = progress.completionDate;

    // --- Debug Log ---
    console.log(`Certificate Check for ${courseId}: Status='${progress.status}', Grade='${displayGrade}', CompletionDate='${completionDate}'`);
    // Use displayGrade for the check
    const shouldShowCert = isCourseCompleted && displayGrade && displayGrade !== 'F';
    ...
    if (shouldShowCert) {
     ...
     } else if (isCourseCompleted && displayGrade === 'F') { // Use displayGrade
         certificateHtml = `<div class="content-card bg-red-50 dark:bg-red-900/30 border-red-300 dark:border-red-600 border"><p class="text-red-700 dark:text-red-200 text-center font-medium">Course completed, but grade does not meet requirements for a certificate.</p></div>`;
          ...
          console.log(`Conditions NOT met: isCourseCompleted=${isCourseCompleted}, grade=${displayGrade}`);
     }
    ...
    <div class="animate-fade-in space-y-6">
        ...
        <div class="content-card grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="text-center p-4 border dark:border-gray-600 rounded">
                ...
                <p class="text-xs text-muted">(Based on daily activity)</p>
            </div>
            <div class="text-center p-4 border dark:border-gray-600 rounded">
        ...
        <div class="content-card">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-[250px]">
                 ...
                 <div>
                     <h4 class="text-sm font-medium text-center mb-2 text-gray-600 dark:text-gray-300">Final Exam Scores (%)</h4>
                     <div class="h-60 relative"><canvas id="finalScoresChart"></canvas></div>
                 </div>
            </div>
        </div>
        ...
        <div class="content-card overflow-x-auto">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ${generateScoreTableRow('Chapters Studied', progress.courseStudiedChapters, courseDef.totalChapters)}
...
// ... (regenerateCertificatePreview remains the same) ...
...
// ... (downloadCertImage, downloadCertPdf remain the same) ...
...
export function renderCourseCharts(progress) {
    ...
    if (assignmentCtx) {
        ...
        // Sort keys numerically based on 'dayX' pattern
        const labels = Object.keys(scores).sort((a, b) => (parseInt(a.replace('day', '')) || 0) - (parseInt(b.replace('day', '')) || 0));
        ...
        if (labels.length > 0) {
            newCharts.assignmentScoresChart = new Chart(assignmentCtx, {
                type: 'line',
                data: { labels: labels.map(l => `D${l.replace('day', '')}`), datasets: [{ label: 'Score', data: dataPoints, borderColor: 'rgb(59, 130, 246)', fill: false, spanGaps: false }] }, // spanGaps: false
    ...
    // --- REMOVED Skip Exam Chart ---
    // const skipCtx = document.getElementById('skipScoresChart')?.getContext('2d');
    // if (skipCtx) { drawNoDataMessage(skipCtx, "Skip exam scores removed from chart."); }

    // --- NEW: Final Exam Scores Chart ---
    const finalCtx = document.getElementById('finalScoresChart')?.getContext('2d');
    if (finalCtx) {
         const scores = Array.isArray(progress.finalExamScores) ? progress.finalExamScores : [];
         const labels = scores.map((_, index) => `Final ${index + 1}`);
         const dataPoints = scores.map(score => (score !== null && score !== undefined ? score : NaN)); // Use NaN for missing attempts
         if (labels.length > 0) {
             newCharts.finalScoresChart = new Chart(finalCtx, {
                 type: 'bar',
                 data: { labels: labels, datasets: [{ label: 'Score', data: dataPoints, backgroundColor: 'rgba(139, 92, 246, 0.7)', borderColor: 'rgb(139, 92, 246)' }] },
                 options: { ...commonOptions }
             });
         } else { drawNoDataMessage(finalCtx, "No final exam scores yet."); }
    }
...
// Updated to handle the "Chapters Studied" row
function generateScoreTableRow(label, scores, totalPossible = null) {
    let count = 0;
    let sum = 0;
    let details = '';
    let validScores = [];
    ...
    if (label === 'Chapters Studied') {
         // Special handling for chapters studied
         count = Array.isArray(scores) ? scores.length : 0;
         totalPossible = totalPossible || 1; // Avoid division by zero
         average = totalPossible > 0 ? ((count / totalPossible) * 100).toFixed(1) : '0.0';
         details = `${count} / ${totalPossible || '?'} Chapters`;
         return `
              <tr>
                  ...
                  <td class="px-4 py-2 text-right text-gray-700 dark:text-gray-200">${average}%</td>
              </tr>
         `;
    } else if (Array.isArray(scores)) {
        // Handle arrays (like finalExamScores)
        validScores = scores.filter(s => s !== null && s !== undefined && !isNaN(s));
        count = validScores.length;
        details = `${count} item${count !== 1 ? 's' : ''}`;
    } else if (scores && typeof scores === 'object') {
        // Handle objects (like assignmentScores, weeklyScores, etc.)
        validScores = Object.values(scores).filter(val => val !== null && val !== undefined && !isNaN(val));
        count = validScores.length;
        details = `${count} item${count !== 1 ? 's' : ''}`;
    } else {
         details = 'N/A';
