// ui_courses.js
// CORRECTED: Added updateGlobalCourseData to the import from state.js
...
// REMOVED navigateToCourseDashboard import (moved to ui_course_dashboard.js)

// --- Course Data Functions (Firestore interaction) ---
async function fetchCourses(searchTerm = '', searchTag = '') {
    let query = db.collection('courses');
    if (currentUser?.uid !== ADMIN_UID) { query = query.where('status', '==', 'approved'); }
    query = query.orderBy('name');
    try {
        const snapshot = await query.limit(100).get();
        let courses = [];
        snapshot.forEach(doc => { courses.push({ id: doc.id, ...doc.data() }); });
        if (!courses.some(c => c.id === FOP_COURSE_ID)) {
            ...
            if(matchesSearch) {
                courses.push({ ...fopDef, status: 'approved' });
        ...
        // Update global state map with fetched courses
        courses.forEach(course => {
            // *** This line requires the imported function ***
...
async function fetchCourseDetails(courseId) {
    ...
    if (courseId === FOP_COURSE_ID) {
        console.log(`Course ${courseId} not found in Firestore, using local config.`);
        // *** This line requires the imported function ***
        updateGlobalCourseData(courseId, FOP_COURSE_DEFINITION);
        return FOP_COURSE_DEFINITION;
    }
...
async function addCourseToFirestore(courseData) {
     ...
     try {
         const chapters = courseData.chapters || (courseData.id === FOP_COURSE_ID ? FOP_COURSE_DEFINITION.chapters : []);
         const docRef = await db.collection('courses').add({
             ...courseData, chapters: chapters, creatorUid: currentUser.uid,
             creatorName: currentUser.displayName || currentUser.email,
             createdAt: firebase.firestore.FieldValue.serverTimestamp(), status: status,
             reportedBy: [], reportReason: null,
             youtubePlaylistUrl: courseData.youtubePlaylistUrl || null,
         ...
         // *** This line requires the imported function ***
         updateGlobalCourseData(docRef.id, { id: docRef.id, ...courseData, chapters, status });
...
function renderCourseList(courses, container) {
    ...
    container.innerHTML = courses.map(course => {
        ...
        const numChapters = Array.isArray(course.chapters) ? course.chapters.length : (typeof course.chapters === 'object' ? Object.keys(course.chapters).length : course.totalChapters || 0);
...
export function showAddCourseForm() {
    ...
    <div class="max-w-lg mx-auto animate-fade-in">
         ...
         <form id="add-course-form" onsubmit="window.submitNewCourse(event)" class="space-y-4 content-card">
              ...
              <div><label for="course-playlist-url">Main YouTube Playlist URL</label><input id="course-playlist-url" type="url"></div>
              ...
              <div><label for="course-trans-pattern">Transcription Path Pattern</label><input id="course-trans-pattern" type="text" placeholder="./Course Transcriptions/chapter{num}.txt"><p class="form-help-text">Use '{num}' as placeholder.</p></div>
...
export async function showEditCourseForm(courseId) {
    ...
    <div class="max-w-lg mx-auto animate-fade-in">
         ...
         <form id="edit-course-form" onsubmit="window.handleUpdateCourse(event, '${courseId}')" class="space-y-4 content-card">
              ...
              <div><label for="edit-course-playlist-url">Main YouTube Playlist URL</label><input id="edit-course-playlist-url" type="url" value="${escapeHtml(course.youtubePlaylistUrl || '')}"></div>
              ...
              <div><label for="edit-course-trans-pattern">Transcription Path Pattern</label><input id="edit-course-trans-pattern" type="text" value="${escapeHtml(course.transcriptionPathPattern || '')}" placeholder="./Course Transcriptions/chapter{num}.txt"></div>
              <p class="text-xs text-muted">(Editing specific chapter resource overrides coming soon)</p>
...
export async function submitNewCourse(event) {
    ...
    if (currentUser?.uid !== ADMIN_UID) { alert("Permission denied."); return; }
    const courseData = {
        name: document.getElementById('course-name')?.value.trim(), description: document.getElementById('course-desc')?.value.trim() || null,
        majorTag: document.getElementById('course-major-tag')?.value.trim() || null, subjectTag: document.getElementById('course-subject-tag')?.value.trim() || null,
        relatedSubjectId: document.getElementById('course-subject-id')?.value.trim(), totalChapters: parseInt(document.getElementById('course-total-chapters')?.value || '0'),
        youtubePlaylistUrl: document.getElementById('course-playlist-url')?.value.trim() || null, pdfPathPattern: document.getElementById('course-pdf-pattern')?.value.trim() || null,
        transcriptionPathPattern: document.getElementById('course-trans-pattern')?.value.trim() || null, chapters: [], chapterResources: {}
    };
    ...
    if (result.success) { alert(`Course "${courseData.name}" added and approved successfully!`); showBrowseCourses(); } else { alert("Failed to add course: " + result.message); }
}
...
export async function handleUpdateCourse(event, courseId) {
    ...
    const updates = {
        ...
        totalChapters: parseInt(document.getElementById('edit-course-total-chapters')?.value || '0'), // Potential NaN here
        youtubePlaylistUrl: document.getElementById('edit-course-playlist-url')?.value.trim() || null,
    ...
    // --- Add Logging ---
    // Log the data being sent just before the Firestore call
...
export async function handleCourseApproval(courseId, approve, deleteCourse = false) {
     ...
     if (approve && !deleteCourse) { const courseData = globalCourseDataMap.get(courseId) || (await db.collection('courses').doc(courseId).get()).data(); if (courseData?.status === 'reported') actionText = 'approve (clear report)'; }
     const confirmationMessage = `Are you sure you want to ${actionText} course ${courseId}?` + (deleteCourse ? '\n\nTHIS ACTION IS PERMANENT AND CANNOT BE UNDONE.' : '');
     ...
     try {
         ...
         hideLoading(); if (success) { handleCourseSearch(); }
