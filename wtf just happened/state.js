...
export let charts = {}; // For the standard progress dashboard
...
// Cache for globally generated content (Formula Sheets, Summaries)
// Key: `${courseId}_${contentType}_ch${chapterNum}`, Value: contentText
export let generatedContentCache = new Map();


// --- State Modifiers ---
export function setAuth(newAuth) { auth = newAuth; }
export function setDb(newDb) { db = newDb; }
export function setData(newData) { data = newData; }
export function setCurrentUser(newUser) { currentUser = newUser; }
export function setCurrentSubject(newSubject) { currentSubject = newSubject; }
export function setCharts(newCharts) { charts = newCharts; }
export function setCurrentOnlineTestState(newState) { currentOnlineTestState = newState; }
export function setUserCourseProgressMap(newMap) { userCourseProgressMap = newMap; }
export function setGlobalCourseDataMap(newMap) { globalCourseDataMap = newMap; }
export function setActiveCourseId(newId) { activeCourseId = newId; }
// NEW: Function to update the global content cache
export function updateGeneratedContentCache(key, content) { generatedContentCache.set(key, content); }
...
export function clearUserSession() {
    ...
    generatedContentCache.clear(); // Clear generated content cache on logout
...
/* userCourseProgress/{userId}/courses/{courseId} document structure:
{
    ...
    skipExamScores: { [chapterNum]: number }, // Stored scores from skip exams
    ...
    myNotes: { [chapterNum]: { submittedAt: timestamp, noteRef: string, originalFilename: string, aiRating: number|null, aiFeedback: string|null, latexSource: string|null } } // Added for My Notes
}*/
