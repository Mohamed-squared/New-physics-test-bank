...
import { showCourseStudyMaterial } from './ui_course_study_material.js';
...
// CORRECT: Import determineNextTask from course_logic.js
import { determineTodaysObjective, getLetterGradeColor, calculateAttendanceScore, determineNextTask } from './course_logic.js';

// --- Navigation ---
// MOVED from ui_courses.js
...
export function showMyCoursesDashboard() {
    if (!currentUser) {
        ...
        // Optionally redirect to login or show an appropriate message
        return;
    }
    setActiveSidebarLink('showMyCoursesDashboard', 'sidebar-standard-nav'); // Link from standard nav
    ...
    userCourseProgressMap.forEach((progress, courseId) => {
        ...
        // Basic progress calculation (e.g., based on studied chapters)
                 ...
                 <div class="flex flex-col sm:flex-row justify-between items-start gap-3">
                     ...
                     <div class="flex-shrink-0 text-right space-y-2 mt-2 sm:mt-0">
                          <button onclick="window.navigateToCourseDashboard('${courseId}')" class="btn-primary-small w-full sm:w-auto">Enter Course</button>
                           <button onclick="window.showCourseProgressDetails('${courseId}')" class="btn-secondary-small w-full sm:w-auto">View Progress</button>
                     </div>
                 </div>
             </div>
        `;
...
export function showCourseDashboard(courseId) {
    ...
    setActiveCourseId(courseId); // Set this as the active course in the state
    setActiveSidebarLink('showCurrentCourseDashboard', 'sidebar-course-nav'); // Activate course nav link
    ...
    if (!progress || !courseDef) {
        ...
        setActiveCourseId(null); // Reset active course ID on error
        setActiveSidebarLink('showMyCoursesDashboard', 'sidebar-standard-nav'); // Revert sidebar link
        return;
    }

    const todaysObjective = determineTodaysObjective(progress, courseDef);
    // Use the imported determineNextTask function
    const nextTask = determineNextTask(progress, courseDef);

    const html = `
        <div id="course-dashboard-area" class="animate-fade-in space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                 <button onclick="window.showCurrentStudyMaterial('${courseId}')" class="content-card text-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors p-4 duration-150 ease-in-out">
                    ...
                    <p class="font-semibold">Study Material</p><p class="text-xs text-muted">(Lectures, PDF, Notes)</p>
                 </button>
                 ...
                 <button onclick="window.showCurrentCourseProgress('${courseId}')" class="content-card text-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors p-4 duration-150 ease-in-out">
    ...
    `;
    // Use displayContent to render into the specific course dashboard area
...
export function handleCourseAction(courseId, actionType, actionId) {
    ...
    switch(actionType) {
        case 'study':
            // actionId should be the chapter number
            showCourseStudyMaterial(courseId, parseInt(actionId));
            break;
        case 'assignment':
        case 'midcourse':
        case 'weekly_exam':
        case 'final':
             // actionId might be 'day1', 'mid1', 'week1', 'final1' etc.
             startAssignmentOrExam(courseId, actionType, actionId);
            break;
        case 'review': // Example action for revision day
             // Navigate to progress page or a dedicated review interface
             showCourseProgressDetails(courseId);
             break;
        case 'completed': // Example action for completed course
             // Show final results/progress
             showCourseProgressDetails(courseId);
             break;
        default:
...
export function showCurrentCourseDashboard() {
    if (activeCourseId) {
        ...
        showMyCoursesDashboard(); // Show list if no course is active
    }
}

/**
 * Shows the study material for the target chapter of the active course.
 * @param {string} [courseId=activeCourseId] - Optional override for course ID.
 */
export function showCurrentStudyMaterial(courseId = activeCourseId) {
     if (!courseId) {
         console.warn("No active course for study material.");
         // Optionally show 'My Courses' or an error message
         return;
     }
     const progress = userCourseProgressMap.get(courseId);
     const courseDef = globalCourseDataMap.get(courseId);
     // Determine target chapter based on progress, default to 1
     const targetChapter = progress && courseDef ? determineTargetChapter(progress, courseDef) : 1;
     showCourseStudyMaterial(courseId, targetChapter); // Show material for the target chapter
}
...
// Make sure functions called directly from HTML are globally accessible.
window.navigateToCourseDashboard = navigateToCourseDashboard;
window.handleCourseAction = handleCourseAction;
window.showCurrentStudyMaterial = showCurrentStudyMaterial; // For dashboard button
window.showCurrentAssignmentsExams = showCurrentAssignmentsExams; // For dashboard button
window.showCurrentCourseProgress = showCourseProgressDetails; // For dashboard button
