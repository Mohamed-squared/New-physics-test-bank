...
// REMOVED: Self-import was causing redeclaration error
// import { showMyCoursesDashboard } from './ui_course_dashboard.js';
import { showCourseStudyMaterial } from './ui_course_study_material.js'; // Keep for potential renaming later
...
// NOTE: We can still call showMyCoursesDashboard directly within this file
 ...
 */
export function showMyCoursesDashboard() { // <--- Exporting the function
    if (!currentUser) {
    ...
    const html = `
        <div id="course-dashboard-area" class="animate-fade-in space-y-6 relative"> <!-- Added relative positioning -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                 <!-- RENAMED Function Call for First Button -->
                 ...
                 <!-- NEW Button for Full Study Material -->
                 <button onclick="window.showFullStudyMaterial('${courseId}')" class="content-card text-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors p-4 duration-150 ease-in-out">
                 ...
                 <!-- Progress button remains the same -->
                 <button onclick="window.showCurrentCourseProgress('${courseId}')" class="content-card text-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors p-4 duration-150 ease-in-out md:col-start-3"> <!-- Adjust column start for layout -->
...
export function handleCourseAction(courseId, actionType, actionId) {
    ...
    switch(actionType) {
        case 'study':
            // Call showCourseStudyMaterial as it handles displaying a specific chapter
...
export function showFullStudyMaterial(courseId = activeCourseId) {
     ...
     // This function will be implemented in ui_course_content_menu.js
     // For now, let's add a placeholder display.
     displayContent(`<h2>Full Study Material Menu for Course ${courseId}</h2><p>Chapter list and exam links will go here.</p>`, 'course-dashboard-area');
     // window.displayCourseContentMenu(courseId); // Future call
